...
public class Paging {
	...
	// 카테고리 목록의 전체 행을 구하는 메서드
	public int getLastPageByCategory() throws Exception {
		//전체 페이지를 담는 변수
		int totalPage = 0;
		//Dao객체 접근
		ProductDao productDao = new ProductDao();
		totalPage = productDao.selectCountByCategory();
		int lastPage = totalPage/rowPerPage;		
		if(totalPage%rowPerPage != 0) {
			lastPage += 1;
		}	
		//System.out.println(lastPage + "<--카테고리 마지막 페이지");
		return lastPage;
	}
